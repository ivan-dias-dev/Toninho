...
const updateProduct = async (id, newbody) => {
  ...
  if (compra) { 
    await Compra.updateOne({ _id: id }, { $set: newbody });
    await User.updateOne({ _id: compra.userId, "compras._id": id }, { $set: { "compras.$.status": newbody.status } });
    const compraAtualizada = await Compra.findOne({ _id: id });
